...
The public homepage, `/online-quote-maker`, `/quotation-software-south-africa`, and `/free-quotation-template` are included in `sitemap.xml`. Each has a self-referencing canonical on `https://quotevia.co.za`, index/follow metadata, and Open Graph and Twitter metadata. The homepage also includes linked Organization, WebSite, WebPage, and SoftwareApplication JSON-LD. The Organization node identifies the online business and its crawlable logo without claiming an unverified physical location, opening hours, reviews, or ratings. `robots.txt` references the sitemap and keeps API and authenticated application paths out of crawl queues. API and secure Quote responses also receive `X-Robots-Tag: noindex, nofollow`, and requests reaching the app on `www.quotevia.co.za` permanently redirect to the canonical non-www origin.

The two acquisition pages are static Server Components with unique search intent, one H1 each, useful workflow and service-business content, visible FAQs, and links among the public marketing routes. They add no client-side state, third-party scripts, or page-specific structured data.

The free quotation template is a value-first public resource. Visitors can edit its highlighted fields directly in the browser, enter their own calculated totals, and use the native print dialog to print or save a local PDF. QuoteVia does not submit, calculate, upload, or persist the template contents. Print-media CSS limits printed output to the quotation document, and the only page-specific Client Component is the button that invokes `window.print()`.
